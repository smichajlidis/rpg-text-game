...
AltarOfLastResortSuccess::AltarOfLastResortSuccess(std::shared_ptr<Player> player, std::shared_ptr<GameState> game_state, const std::string& description, const std::string& choice_1, const std::string& choice_2)
    ...
    addNewItem("As you kneel in prayer before the altar, a radiant light descends from above, illuminating a legendary sword nestled in the stone—a gift from the gods to aid you in your quest. Behold, Excalibur, a blade of unparalleled might that increases your strength by 50 with each swing.", "excalibur", 700, "weapon", 50);
    addNewItem("In the hallowed silence of the chapel, the presence of the ancients envelops you, bestowing upon you a suit of armor forged from divine essence. The Aegis of the Ancients, an indestructible shield against all odds, enhances your endurance by 50, a testament to the gods' protection.", "aegis_of_the_ancients", 450, "armor", 50);
    ...
    addNewItem("With each whispered entreaty, the crown upon the altar begins to glow with an ethereal light—a sign of the gods' favor. This is the Crown of Charm, an adornment that enhances your charisma by 50, allowing you to sway hearts and minds with ease.", "crown_of_charm", 800, "charisma_amulet", 50);
    addNewItem("In a moment of divine grace, a gleaming amulet materializes before you, radiating with the blessings of fortune. This is Fortune's Amulet, a talisman imbued with luck, increasing your fortune by 50 and guiding your path to prosperity.", "fortune_amulet", 320, "luck_amulet", 50);

    related_locations.push_back("altar_of_last_resort");
    related_locations.push_back("chapel_altars");
}

void AltarOfLastResortSuccess::addNewItem(std::string description, std::string name, std::uint32_t value, const std::string& type, std::uint32_t strength) {
