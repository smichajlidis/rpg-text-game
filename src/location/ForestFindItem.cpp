...
ForestFindItem::ForestFindItem(std::shared_ptr<Player> player, std::shared_ptr<GameState> game_state, const std::string& description, const std::string& choice_1,  const std::string& choice_2)
    ...
    addNewItem("While wandering through the dense forest, you stumble upon an old, rusty sword partially buried in the ground. Despite its worn appearance, it still seems sturdy enough for use.", "rusty_sword", 55, "weapon", 17);
    ...
    addNewItem("Scattered across the forest floor are clusters of plump wild berries, ripe for the picking. Though common, their sweet flavor and nutritional value make them a welcome find for any weary traveler.", "wild_berries", 20, "food", 15);
...
void ForestFindItem::addNewItem(std::string description, std::string name, std::uint32_t value, const std::string& type, std::uint32_t strength) {
