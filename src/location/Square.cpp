...
Square::Square(std::shared_ptr<GameState> game_state, const std::string& description, const std::string& choice_1, const std::string& choice_2, const std::string& choice_3)
    ...
    game_state->addLocation("tavern", std::make_shared<Tavern>(game_state->getPlayer(), game_state));
    game_state->addLocation("forest", std::make_shared<Forest>(game_state->getPlayer(), game_state));
